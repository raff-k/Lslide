...
#' @param angle default: 0 degree for Noth-South. For East-West use 90 degree.
...
#' @export
#'
dividePolygon <- function(x, angle = 0, n, btol = 0.0001, pgtol = 1e-9, maxit = 100, cores = 1, quiet = TRUE)
{
  ...
  # x.div <- lapply(X = 1:n.obj, FUN = function(i, x.sf, crs, angle, n, btol, pgtol, maxit, x.cntrd, rot, rot.inv, opt.fun, splitByLines){
      ...
      # get objects
        ...
        x.i.rot <- sf::st_sf(sf::st_set_geometry(x.i, NULL) , geom = x.i.rot, crs = sf::st_crs(x.i))
        ...
        # split polygon
        # # #
        x.i.rot.div <- splitByLines(x = x.extent, y = y.par.opt, x.i = x.i.rot)
        ...
        x.i.div <- sf::st_sf(sf::st_set_geometry(x.i.rot.div, NULL) , geom = x.i.div, crs = sf::st_crs(x.i.rot.div))
