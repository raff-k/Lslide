...
#' @export
#'
highPassThresholding <- function(x, scale.factor, threshold, path.output = NULL, do.sieve = TRUE, sieve.mode = "0", sieve.thresh = 4, do.shrink.expand = TRUE, expand = 4,
                                 do.buf = FALSE, buf.size = 1, NoData = -99999, env.rsaga = NULL, show.output.on.console = FALSE, quiet = TRUE)
{
  ...
  if(quiet == FALSE) cat("------ Run of highPassThresholding: " , round(x = process.time.run["elapsed"][[1]]/60, digits = 3), " Minutes ------\n")
